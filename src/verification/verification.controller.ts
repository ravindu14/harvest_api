...
import {
  FailToUpdateVerificationException,
  VerificationNotFoundException,
} from "./verification.exceptions";
...
export default class VerificationController implements Controller {
  ...
  private saveOrUpdateVerification = async (
    ...
    if (await this.verification.findOne({ verificationId })) {
      const verification = await this.verification.findOneAndUpdate(
        { verificationId },
        newVerification
      );
  ...
  private getAllVerificationsForFarmer = async (
    ...
    const verifications = await this.verification.find({
      researchCenter,
      username,
    });
